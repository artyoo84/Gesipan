...
public class ThemeDaoImpl implements CommonDAO {
	...
	public int count() {
		...
		try {
			/*
			 * StringBuffer query = new StringBuffer(200); query.append(
			 * "select count(*) from THEME_MESSAGE "); if (whereCond != null &&
			 * whereCond.size() > 0) { query.append("where "); for (int i = 0; i
			 * < whereCond.size(); i++) { query.append(whereCond.get(i)); if (i
			 * < whereCond.size() - 1) { query.append(" or "); } }
			 */
			...
			while (keyIter.hasNext()) {
				...
				if (obj instanceof String) {
					...
					// pstmt.setTimestamp(key.intValue(), (Date) obj);
				}
			}
	...
	public Object getElementById(String id) {
		...
		try {
			...
			if (rsMessage.next()) {
				...
				pstmt = conn.prepareStatement("select CONTENT from THEME_CONTENT " + "where THEME_MESSAGE_ID = ?");
				pstmt.setString(1, id);
				rs = pstmtContent.executeQuery();
				if (rs.next()) {
					// Reader reader = null;
					try {
	...
	public List<Object> list() {
		...
		/*	while (keyIter.hasNext()) {
				...
				if (obj instanceof String) {
					...
					pstmt.setString(key.intValue(), ((Integer) obj).intValue());
				} else if (obj instanceof Date) {
					pstmt.setString(key.intValue(), (Date) obj);
				}
			}*/
